...
import { revenueSeries, niches } from "@/lib/dashboard-data";
...
const kpis = [
  { label: "Revenue", value: "$2,850.75", change: "+14.2%" },
  { label: "Total clicks", value: "38,412", change: "+9.6%" },
  { label: "Conversions", value: "612", change: "+18.1%" },
  { label: "Conv. rate", value: "1.59%", change: "+0.3pt" },
];
    ...
    <div className="animate-fade-up">
      ...
      <section className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        {kpis.map((k, i) => (
          ...
          >
            ...
            <p className="mt-1 text-[12px] font-medium text-success">{k.change}</p>
          </div>
        ))}
      </section>
            ...
            <AreaChart data={revenueSeries} margin={{ top: 10, right: 8, left: -18, bottom: 0 }}>
              <defs>
                <linearGradient id="a1" x1="0" x2="0" y1="0" y2="1">
                  <stop offset="0%" stopColor="#2c5aa0" stopOpacity={0.35} />
                  <stop offset="100%" stopColor="#2c5aa0" stopOpacity={0} />
                </linearGradient>
                <linearGradient id="a2" x1="0" x2="0" y1="0" y2="1">
                  <stop offset="0%" stopColor="#dc3545" stopOpacity={0.28} />
              ...
              <Tooltip
                contentStyle={{ background: "#1b3a6f", border: "none", borderRadius: 8, color: "#fff", fontSize: 12 }}
                labelStyle={{ color: "#a9c1e6", fontSize: 11 }}
              />
      ...
      <section className="mt-6 grid grid-cols-1 gap-4 md:grid-cols-3">
        {platforms.map((p, i) => (
          <div
            key={p.name}
            style={{ animationDelay: `${i * 80}ms`, backgroundColor: p.tint, borderColor: `${p.tone}30` }}
            className="animate-fade-up hover-lift rounded-xl border-2 p-5"
          >
            <p className="font-mono text-[11px] uppercase tracking-wider" style={{ color: p.tone }}>{p.name}</p>
            <p className="mt-3 font-display text-3xl font-semibold" style={{ color: p.tone }}>
      ...
      <section className="mt-6 rounded-xl border border-border bg-card p-5 sm:p-6">
        ...
        <div className="h-[220px]">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={niches.map((n) => ({ name: n.name, revenue: n.revenue, color: n.color }))} margin={{ top: 10, right: 8, left: -12, bottom: 0 }}>
              <CartesianGrid stroke="#f5efe7" vertical={false} />
