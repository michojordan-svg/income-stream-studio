...
import { affiliateLinks as staticLinks } from "@/lib/dashboard-data";
...
// Adapt API shape to what the UI needs
function toDisplay(l: AffiliateLink) {
  return {
    id:          l.id,
    product:     l.product_name,
    slug:        l.shortened_url,
    niche:       l.niche,
    network:     l.affiliate_network ?? "—",
    commission:  `${l.commission_rate}%`,
    clicks:      l.clicks,
    conversions: l.conversions,
    revenue:     typeof l.revenue === "string" ? parseFloat(l.revenue) : l.revenue,
...
function LinksPage() {
  ...
  const { data, isLoading } = useLinks({ limit: 50 });

  const raw = data?.links ?? [];
  const links = raw.length ? raw.map(toDisplay) : staticLinks;
  ...
  const totalRevenue  = summary ? parseFloat(summary.total_revenue)  : links.reduce((s, l) => s + l.revenue, 0);
  const totalClicks   = summary ? parseInt(summary.total_clicks)      : links.reduce((s, l) => s + l.clicks, 0);
  const totalConv     = summary ? parseInt(summary.total_conversions) : links.reduce((s, l) => s + l.conversions, 0);
  const activeCount   = links.filter((l) => l.active).length;

  return (
    <div className="animate-fade-up">
      <PageHeader
        eyebrow={`${links.length} tracked links`}
        ...
        {links.map((link, i) => (
          ...
          >
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0">
                <div className="flex flex-wrap items-center gap-2">
                  <span className={"inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wider " + (link.active ? "bg-success/15 text-success" : "bg-cream-warm text-muted-foreground")}>
                ...
                <p className="mt-1 text-[12px] text-muted-foreground">{link.network} · {link.commission} commission</p>
              </div>
            ...
            <div className="mt-4 flex items-center gap-2 rounded-md border border-border-soft bg-cream-soft px-3 py-2">
              ...
              <code className="min-w-0 flex-1 truncate font-mono text-[12.5px] text-primary">{link.slug}</code>
              <button
              ...
              >
                {copied === link.id ? <><Check className="h-3 w-3 text-success" /> Copied</> : <><Copy className="h-3 w-3" /> Copy</>}
              </button>
            </div>
